...
`TypeError`. Кортеж неизменяем, нельзя использовать `+=` для добавления элементов. Можно было бы создать новый кортеж через конкатенацию: `t = t + ([5, 6],)`.
</details>
...
`return` возвращает значение из функции, которое можно присвоить переменной или использовать дальше. `print` выводит текст на экран, но не возвращает полезного значения (возвращает `None`).
</details>
...
`SyntaxError`. Позиционные аргументы должны идти перед именованными. После `a=1` нельзя использовать позиционные `2, 3`. Правильно: `func(1, 2, 3)` или `func(2, 3, a=1)`.
</details>
...
Лямбда - анонимная однострочная функция: `lambda x: x * 2`. Ограничения: только одно выражение, нельзя использовать statements (if, for, и т.д.). Используются для простых операций в `map`, `filter`, `sorted`.
</details>
